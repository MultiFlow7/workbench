---
project: 工作台
updated: 2026-06-13 (req-066 confirmed · v0.16.1 公开产品洁净度补丁规划中)
---
...
| [req-066](req-066-public-cleanliness-patch.md) | 公开产品洁净度补丁 | critical | confirmed | v0.16.1 |
...
| ID | 标题 | 优先级 | 状态 | 版本 |
|----|------|--------|------|------|
| [req-013](req-013-agent-task-state-machine.md) | Agent 任务状态机（后端） | high | planned | v0.6 |
| [req-041](req-041-tool-calling-framework.md) | Tool Calling 基础框架（桌面主进程 tool executor + ChatView tool call 处理） | high | planned | v0.11 |
| [req-042](req-042-builtin-tools.md) | 内置工具集（读取本地文件 / 执行 Shell 命令 / 搜索 Vault） | high | planned | v0.11 |
| [req-043](req-043-tool-call-ui-feedback.md) | 工具调用状态 UI 反馈（最小化可见进度） | medium | planned | v0.11 |
| [req-044](req-044-p4-text-input-expansion.md) | P4 文本输入展开面板（长文本输入体验优化） | medium | planned | v0.12 |
...
| ID | 标题 | 优先级 | 状态 | 版本 |
|----|------|--------|------|------|
| [req-020](req-020-main-conversation-isolation.md) | 主对话保护（独立于后台任务） | high | in-progress | v0.8 |
...
| [req-024](req-024-per-agent-llm-config.md) | Agent 级别 LLM 配置（每个角色可绑定不同底层模型） | medium | in-progress | v0.9 |
