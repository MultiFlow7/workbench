...
import { buildSafeSidecarEnv } from '../security/safeEnv'
...
export async function startAiService(): Promise<boolean> {
  ...
  _childProcess.stdout?.on('data', (chunk) => {
    process.stdout.write(`[ai-service] ${redactSensitive(chunk.toString())}`)
  })
  _childProcess.stderr?.on('data', (chunk) => {
    process.stderr.write(`[ai-service] ${redactSensitive(chunk.toString())}`)
  })
  ...
  _childProcess.on('error', (err) => {
    console.error('[ai-service] spawn error:', redactSensitive(err))
    _serviceReady = false
  })
...
export function stopAiService(): void {
  ...
  try {
    ...
    console.error('[ai-service] stop error:', redactSensitive(err))
  }
  _childProcess = null
  _serviceReady = false
}
