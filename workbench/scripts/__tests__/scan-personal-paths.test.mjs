...
console.log('T-V016-U5 命中样本（macOS）')
{
  ...
  assert(result.stdout.includes('[macOS]'), 'stdout 含平台标识 [macOS]')
...
console.log('T-V016-U7 三平台 pattern')
{
  ...
  const result = runScan(dir)
  assert(result.code === 1, '退出码 1')
  assert(result.stdout.includes('[macOS]'), 'macOS 平台被识别')
  assert(result.stdout.includes('[Windows]'), 'Windows 平台被识别')
  assert(result.stdout.includes('[Linux]'), 'Linux 平台被识别')
  cleanup(dir)
}
...
console.log('错误处理：target 不存在')
{
  ...
  assert(result.stderr.includes('not found'), 'stderr 含友好错误')
}
