    ...
    if request.stream:
        async def event_stream() -> AsyncGenerator[str, None]:
            try:
                ...
                logger.error("Upstream HTTP error during stream: %s %s", exc.response.status_code, exc)
                yield f"data: {{\"error\": \"Upstream HTTP {exc.response.status_code}\"}}\n\n"
            except httpx.RequestError as exc:
                logger.error("Upstream request error during stream: %s", exc)
                yield "data: {\"error\": \"Upstream connection error\"}\n\n"
            except Exception as exc:
                logger.error("Unexpected error during stream: %s", exc, exc_info=True)
        ...
        try:
            ...
            logger.error("Adapter complete() error: %s", exc)
